        ...
        if (searchStatus) {
            if (options.isSearchComplete) {
                ...
                if (options.isError) {
                    ...
                    if (errorType && window.t) {
                        switch (errorType) {
                            case 'iterationLimit':
                                statusText = t('errors.failedIterationLimit');
                                break;
                            case 'timeLimit':
                                statusText = t('errors.failedTimeLimit');
                                break;
                            case 'depthLimit':
                                statusText = t('errors.failedDepthLimit');
                                break;
                            case 'frontierEmpty':
                                statusText = t('errors.failedFrontierEmpty');
                                break;
                            case 'stopped':
                                statusText = t('errors.failedStopped');
                                break;
                            default:
                                statusText = t('graphs.failed');
                        }
                    } else {
                        // English fallback
                        switch (errorType) {
                            case 'iterationLimit':
                                statusText = 'Failed (Iteration Limit)';
                                break;
                            case 'timeLimit':
                                statusText = 'Failed (Time Limit)';
                                break;
                            case 'depthLimit':
                                statusText = 'Failed (Depth Limit)';
                                break;
                            case 'frontierEmpty':
                                statusText = 'Failed (No Solution)';
                                break;
                            case 'stopped':
                                statusText = 'Failed (Stopped)';
                                break;
                            default:
                                statusText = 'Failed';
                        }
                    ...
                    statusText = window.t ? t('graphs.solved') : 'Solved';
                }
